
/*
 * From Major at Rune-Server
 * https://www.rune-server.ee/runescape-development/rs2-client/snippets/430099-bzip2-classes-refactor.html
 */

/**
 * A Java implementation of the DState struct (structure holding all the
 * decompression-side stuff).
 * 
 * @see "http://svn.apache.org/repos/asf/labs/axmake/trunk/src/libuc++/srclib/bzip2/bzlib_private.h"
 */
class BZip2DecompressionState
{

	public BZip2DecompressionState()
	{
		anIntArray583 = new int[256];
		anIntArray585 = new int[257];
		aBooleanArray589 = new bool[256];
		aBooleanArray590 = new bool[16];
		aByteArray591 = new byte[256];
		aByteArray592 = new byte[4096];
		anIntArray593 = new int[16];
		aByteArray594 = new byte[18002];
		aByteArray595 = new byte[18002];
		aByteArrayArray596 = new byte[6][];
		anIntArrayArray597 = new int[6][];
		anIntArrayArray598 = new int[6][];
		anIntArrayArray599 = new int[6][];
		for(int i = 0; i < 6; i++)
		{
			aByteArrayArray596[i] = new byte[258];
			anIntArrayArray597[i] = new int[258];
			anIntArrayArray598[i] = new int[258];
			anIntArrayArray599[i] = new int[258];
		}
		anIntArray600 = new int[6];
	}

	public byte[] aByteArray563;
	public int anInt564;
	public int anInt565;
	public int anInt566;
	public int anInt567;
	public byte[] aByteArray568;
	public int anInt569;
	public int anInt570;
	public int anInt571;
	public int anInt572;
	public byte aByte573;
	public int anInt574;
	public bool aBoolean575;
	public int anInt576;
	public int anInt577;
	public int anInt578;
	public int anInt579;
	public int anInt580;
	public int anInt581;
	public int anInt582;
	public int[] anIntArray583;
	public int anInt584;
	public int[] anIntArray585;
	public static int[] anIntArray587;
	public int anInt588;
	public bool[] aBooleanArray589;
	public bool[] aBooleanArray590;
	public byte[] aByteArray591;
	public byte[] aByteArray592;
	public int[] anIntArray593;
	public byte[] aByteArray594;
	public byte[] aByteArray595;
	public byte[][] aByteArrayArray596;
	public int[][] anIntArrayArray597;
	public int[][] anIntArrayArray598;
	public int[][] anIntArrayArray599;
	public int[] anIntArray600;
	public int anInt601;
}